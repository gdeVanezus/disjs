...
class Client extends EventEmitterExt {
  constructor(options) {
    ...
    this.ignoreSessionsLimit = !!options.ignoreSessionsLimit;
    this.loggerFactory =
      options.loggerFactory instanceof LoggerFactory
    ...
    let shardIds = Array.isArray(options.shards) ? options.shards : [];
    let shardCount = options.shardCount ?? shardIds.length ?? 0;
  ...
  async run(options = {}) {
    const gatewayInformation = await this.rest
      .request(Endpoints.GET_GATEWAY_BOT())
    ...
    switch (this.shardCount) {
      case -1: {
        shardIds = Array.from({ length: gatewayInformation.shards }, (_, i) => i);
    ...
    const { total, max_concurrency: maxConcurrency, reset_after: resetAfter } =
      gatewayInformation.session_start_limit;

    if (shardIds.length > total) {
      if (!this.ignoreSessionsLimit) {
        this.logger?.fatal(
          `Cannot run bot; not enough sessions: ${shardIds.length}, but have ${total} available. Limit resets on: ${new Date(
            now + resetAfter
          )}.\n` +
        ...
        this.logger?.warn(
          `Not enough sessions: wanted ${shardIds.length}, but have ${total} available. Limit resets on: ${new Date(
            now + resetAfter
          )}.\n` +
    ...
    this.logger?.info(
      `Starting ${this.shards.length} shards... (${total} sessions remaining) (${maxConcurrency} shards/s)`
    );
  ...
  _shardFor(guildId) {
    return BigInt(guildId.value >> 22n) % BigInt(this.shards.length);
  }

  updateVoiceState(guildId, channelId, options = {}) {
    const gid = new Snowflake(guildId);

    return this.shards[Number(this._shardFor(gid))].updateVoiceState(
      gid.toString(),
      new Snowflake(channelId).toString(),
      options
    );
  }

  updatePresence(options) {
    return this.asyncForEachShard(async (gateway) => {
      await gateway.updatePresence(options);
    });
  }
}
